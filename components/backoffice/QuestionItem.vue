...
const selectStatus = (statusOption: QuestionTypeOption) => {
  prop.question.questionType = statusOption.urlSend;
  prop.question.questionTypeName = statusOption.name;
...
<template>
  <div class="relative col-span-1 mt-10 flex gap-5">
    ...
    <Trash
      ...
      @click="removeField(indexName, index, question.questionId)"
    />
    <div
      ...
      <div class="relative">
          ...
          {{
            ...
            question.questionTypeName
              ...
              : question.questionType
                ? statusData.find((x) => x.urlSend === question.questionType)
                    ?.name
                : 'Please select your question type'
          }}
          <Arrow
        ...
        >
          <button
            ...
            :class="`status-choice w-full rounded-md px-4 py-1 text-start font-semibold ${question.questionTypeName === statusOption.name ? selectedEventTimeStyle?.active : 'duration-200 hover:bg-zinc-200'}`"
          >
            {{ statusOption.name }}
          </button>
        </div>
